...
def maybe_download_mnist():
    ...
    for file in mnist_files:
        if not maybe_download('../data/mnist', 'https://www.kaggle.com/datasets/hojjatk/mnist-dataset/data', file):
            continue
...
def load_Loan_data(dtype=np.int64):
    """Load Loan_data"""

    data_dir = '../data/loan'

    train_path = os.path.join(data_dir, 'Loan_train.csv')
    test_path = os.path.join(data_dir,'Loan_test.csv')
    train_label_path = os.path.join(data_dir,'Loan_test_labels.csv')
    test_label_path = os.path.join(data_dir,'Loan_test_labels.csv')
