...
depth = 0
num_repetitions = 1
Points = [300,500,1000,2500,5000,10000]
#Points_ = [10+5,2000+5]
points_str = []
for x in Points:
...
X_orig = x_orig1.reshape([len(x_orig1),2]) 
...
for num_latents in Latents:
    ...
    for points in Points:
        ...
        train_x_orig = train_x_random[:points]
        train_x = train_x_orig.reshape([len(train_x_orig),2])
        ...
        for i in range(len(input_params)):
            if i%2 == 0:
                mu_gen.append(input_params[i].cpu())
            else:
                sigma_gen.append(input_params[i].cpu())
        ...
        x_gen = np.sort(x_gen)  
        X_gen = x_gen#.reshape([len(x_gen),2]) 
        
        Gen_Data = 0 
        for i in range(num_latents):
            Generate_ = multivariate_normal.pdf(X_gen,mu_gen[i],sigma_gen[i])
            ...
            plt.savefig('2D_Gaussian_'+str(depth)+'_'+str(num_latents)+'_'+str(points)) 
        """
...
plt.ylabel('Degree of overfitting')
# Plotting
plt.plot(Peakness[0], Deg_Over[0] , label='Latents = 10',marker = 'o', markersize=15)
plt.plot(Peakness[1], Deg_Over[1] , label='Latents = 20',marker = 'o', markersize=15)
plt.plot(Peakness[2], Deg_Over[2] , label='Latents = 30',marker = 'o', markersize=15)
plt.plot(Peakness[3], Deg_Over[3] , label='Latents = 50',marker = 'o', markersize=15)
plt.title('Degree of Overfitting vs flatness')
plt.legend()
plt.savefig('Degree of Overfitting vs flatness_across latents')
plt.close()     
