...
Valid_data = valid_dataset.data[0:4000]
...
for size in Train_size:
    ...
    valid_data = Valid_data.reshape(4000, 28*28)
    ...
    for num_latents in Latents:
        TRAIN_like = np.zeros(1000)
        TEST_like = np.zeros(1000)
        ...
        plt.legend()
        plt.savefig(' likelihood_28_256_'+str(num_latents)+'_'+str(depth))
        plt.close()     
