...
for depth in Depths:
    ...
    for ins in range(instance):
        ...
        print('dim = ',train_x.to(pc.device).dim())
        print(train_x.to(pc.device))
        lls = pc(train_x.to(pc.device))
        train_LL = lls.mean().detach().cpu().numpy().item()
    ...
    plt.legend()
    plt.savefig('SPN_flatness_max_'+ dataset+'_'+str(num_latents)+'_'+ str(depth)+'_'+ str(num_repetitions))
    plt.close()
    ...
    ax1.set_ylim([ymin-0.5,ymax+0.004]) 
    ...
    plt.savefig('SPN_ll_flat_max_bar_'+ dataset+'_'+str(num_latents)+'_'+ str(depth)+'_'+ str(num_repetitions))
    plt.close()
    ...
    ymin = min(flatness)
    ymax = max(flatness)
    ax2.set_ylim([ymin/2,ymax+0.004]) 
    ax2.legend(loc='upper right')
    plt.title('SPN likelyhood flatness')
    plt.savefig('SPN_llDiff_flat_max_bar_'+ dataset+'_'+str(num_latents)+'_'+ str(depth)+'_'+ str(num_repetitions))
    plt.close()
    
    d = OrderedDict(zip(abs(blue_bar-orange_bar), flatness))
    res = list(OrderedDict(sorted(d.items(), key=lambda x: x[1])))
    plt.figure(figsize = (15,12))
    plt.plot(np.sort(np.array(flatness)),res,color = 'red')
    plt.xlabel('flatness')
    plt.ylabel('difference in likelihood')
    plt.title('Flatness of minima vs likelihood')
    plt.savefig('SPN_flatness_max_ll_'+ dataset+'_'+str(num_latents)+'_'+ str(depth)+'_'+ str(num_repetitions))
    plt.close()    
